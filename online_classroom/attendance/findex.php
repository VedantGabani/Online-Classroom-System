...
if (isset($_POST) && !empty($_POST)) {
    $numbers = explode(' ', $_POST['exception_numbers']);
    $classID = $_POST['classNames'];
    $all = $_POST['groupStatus'];

    $classHourAddition = "UPDATE class_master SET hours = hours+1 WHERE id = $classID";
    $con->query($classHourAddition);

    if ($_POST['exception_numbers'] == null || trim($_POST['exception_numbers']) == "") {
        if ($all == 1) {
            $addInStudents = "UPDATE student_master SET hours = hours + 1 WHERE class_id = $classID";
            $con->query($addInStudents);
        }
    } else {
        if ($all == 1) {
            $addInStudents = "UPDATE student_master SET hours = hours + 1 WHERE class_id = $classID";
            ...
            foreach ($numbers as $num) {
                $deductStudent = "UPDATE student_master SET hours = hours - 1 WHERE enroll = $num";
                $con->query($deductStudent);
            }
        } else if ($all == 0) {
            foreach ($numbers as $num) {
                $addStudent = "UPDATE student_master SET hours = hours + 1 WHERE enroll = $num";
                $con->query($addStudent);
            }
        }
    }
}
        ...
        <div class="container-fluid"><a class="navbar-brand" href="../index.html">CO INTRA</a><button class="navbar-toggler" data-toggle="collapse" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
            <div class="collapse navbar-collapse" id="navcol-1">
                <ul class="nav navbar-nav mx-auto">
                    ...
                    <li class="nav-item" role="presentation"><a class="nav-link" href="../assignment/findex.php">Flash Announcement</a></li>
                </ul>
                <ul class="nav navbar-nav">
                    <li class="nav-item" role="presentation"><a class="nav-link" href="../login/logout.php">Logout</a></li>
                </ul>
            </div>
        </div>
    </nav>
                ...
                <div class="col">
                    <form action="" method="POST">
                        ...
                        <div class="container">
                            <h2>Attendance</h2>
                            <div class="radio">
                                <label><input name="groupStatus" type="radio" name="optradio" value="1" require>All Present</label>
                            </div>
                            <div class="radio">
                                <label><input name="groupStatus" type="radio" name="optradio" value="0">All Absent</label>
                            </div>
                            <div class="form-group">
                                <label for="list">Except these numbers:</label>
                                <textarea name="exception_numbers" class="form-control" rows="5" id="list"></textarea>
                            </div>
